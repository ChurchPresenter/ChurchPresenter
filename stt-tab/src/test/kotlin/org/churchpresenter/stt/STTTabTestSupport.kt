 ...
 *
 * The tab is driven through a real [STTManager] and a [FakeSttOutput]. Neither needs a
...
/**
 * Composes `STTTab` over a real [STTManager] and a [FakeSttOutput] and runs [block].
 *
...
internal fun sttTab(
    ...
    block: ComposeUiTest.(stt: STTManager, output: FakeSttOutput, reports: STTReports) -> Unit,
) {
    val appSettings = AppSettings(sttSettings = settings)
    val stt = STTManager()
    val output = FakeSttOutput()
