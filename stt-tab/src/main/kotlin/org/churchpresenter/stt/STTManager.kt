...
class STTManager(
    /**
     * The shared 30-day retention sweep over `~/.churchpresenter/bible-stt-logs/`.
     *
     * The `.db` snapshots this class captures land in the same folder the app's Bible training log
     * writes to, and share its retention policy. That log is about Bible references, not captions,
     * so it stays in `:composeApp` — the app passes `TrainingDataLogger::cleanupOldLogsOnce` here.
     * Defaulted to doing nothing so the module and its tests need no such folder at all.
     */
    private val cleanupSharedLogs: () -> Unit = {},
) {
     ...
     *
     * Public rather than `internal` because it is the transition the socket's own `connect` callback
     * runs, and consumers outside this module need to reach a connected manager in a test without
     * standing up an STT server — the Bible tab's auto-follow states are only reachable that way.
     * Nothing in production calls it except the callback.
     */
    fun applyConnected() {
    ...
    internal fun captureDbSnapshot(baseUrl: String) {
        // Shares the app's 30-day retention sweep so these .db snapshots don't accumulate forever
        // in bible-stt-logs. Injected rather than called directly: the log itself is Bible training
        // data and stays in :composeApp — see [cleanupSharedLogs].
        cleanupSharedLogs()
