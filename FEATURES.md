...
- `ndi/` (the `:ndi` Gradle module) — NDI itself: `NdiRuntime` (finding the separately installed runtime), `NdiLibrary`/`JnaNdiLibrary` (the six native calls), `NdiSender` and `NdiOutputMode`
- `presenter/NdiVideoRenderer.kt`, `presenter/NdiManager.kt` — the app-side wiring, and `dialogs/tabs/ProjectionNdiCard.kt` for its settings card
