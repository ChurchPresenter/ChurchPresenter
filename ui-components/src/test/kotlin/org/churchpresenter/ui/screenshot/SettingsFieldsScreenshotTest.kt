...
class SettingsFieldsScreenshotTest {
    ...
    /**
     * The verse the font picker's preview box quotes, pinned.
     *
     * [FontPreviewText] is a process-wide singleton the running app fills in as translations load,
     * and nothing in this module loads one — so left alone the picker falls back to its script
     * specimen, and the image would instead record whichever test last wrote to the singleton.
     */
    @BeforeTest
    fun pinPreviewVerse() =
        FontPreviewText.update(listOf("In the beginning God created the heaven and the earth."))

    @AfterTest
    fun forgetPreviewVerse() = FontPreviewText.clear()
