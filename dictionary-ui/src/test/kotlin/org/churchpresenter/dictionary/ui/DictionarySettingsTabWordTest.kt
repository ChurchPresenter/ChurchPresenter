...
class DictionarySettingsTabWordTest {
    ...
    @Test
    fun `the show switch turns the word off and back on`() = dictionarySettingsTab { get ->
    ...
    fun `the word colour field stores the confirmed hex`() {
        dictionarySettingsTab(initial = dictionarySettings { copy(wordColor = "#101112") }) { get ->
    ...
    @Test
    fun `the bold button stores the word as bold`() = dictionarySettingsTab { get ->
    ...
    @Test
    fun `the italic button stores the word as italic`() = dictionarySettingsTab { get ->
     ...
     */
    @Test
    fun `the underline button is rendered but wired to nothing`() = dictionarySettingsTab { get ->
    ...
    @Test
    fun `the shadow button stores the flag and reveals the shadow row`() = dictionarySettingsTab { get ->
    ...
    fun `the word shadow colour stores the confirmed hex`() {
        dictionarySettingsTab(initial = dictionarySettings { copy(wordShadow = true, wordShadowColor = "#123456") }) { get ->
    ...
    fun `the word shadow size stores a new percentage`() {
        dictionarySettingsTab(initial = dictionarySettings { copy(wordShadow = true, wordShadowSize = 111) }) { get ->
    ...
    fun `the word shadow intensity stores a new percentage`() {
        dictionarySettingsTab(initial = dictionarySettings { copy(wordShadow = true, wordShadowOpacity = 77) }) { get ->
    ...
    fun `the word font dropdown stores the picked family`() {
        ...
        dictionarySettingsTab(initial = dictionarySettings { copy(wordFontType = SENTINEL_FONT) }) { get ->
    ...
    fun `clicking the font field opens the panel and a picked font is stored`() {
        ...
        dictionarySettingsTab(initial = settings) { get ->
    ...
    @Test
    fun `the word font size stores a new value`() = dictionarySettingsTab { get ->
     ...
     */
    @Test
    fun `a word font size outside the range is not stored`() = dictionarySettingsTab { get ->
    ...
    fun `a typed word font size is what a fresh render of the saved settings shows`() {
        ...
        dictionarySettingsTab(initial = dictionarySettings { copy(wordFontSize = saved) }) { _ ->
