...
class DictionarySettingsTabStructureTest {
    ...
    @Test
    fun `the tab renders every section exactly once`() = dictionarySettingsTab { _ ->
    ...
    @Test
    fun `the tab offers one switch per toggleable setting`() = dictionarySettingsTab { _ ->
    ...
    fun `each switch ordinal addresses the setting it is named for`() {
        ...
        for ((ordinal, settings) in cases) {
            dictionarySettingsTab(initial = settings) { _ ->
    ...
    @Test
    fun `the tab offers one colour field per colour setting`() = dictionarySettingsTab { _ ->
    ...
    fun `switching both shadows on adds their colour fields`() {
        dictionarySettingsTab(initial = dictionarySettings { copy(wordShadow = true, referenceShadow = true) }) { _ ->
            colorFields().assertCountEquals(7)
        }
    }

    @Test
    fun `the tab offers one number field per numeric setting`() = dictionarySettingsTab { _ ->
    ...
    @Test
    fun `only the word and reference sections offer a font dropdown`() = dictionarySettingsTab { _ ->
    ...
    @Test
    fun `both style groups offer all four buttons`() = dictionarySettingsTab { _ ->
    ...
    @Test
    fun `the tab offers a captioned slider for opacity and for duration`() = dictionarySettingsTab { get ->
