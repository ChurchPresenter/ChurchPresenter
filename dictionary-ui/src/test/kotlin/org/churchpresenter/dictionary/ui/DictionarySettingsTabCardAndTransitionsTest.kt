...
class DictionarySettingsTabCardAndTransitionsTest {
    ...
    fun `the card background colour field stores the confirmed hex`() {
        dictionarySettingsTab(initial = dictionarySettings { copy(cardBackgroundColor = "#2B2D42") }) { get ->
    ...
    @Test
    fun `the opacity readout starts on the stored value`() = dictionarySettingsTab { get ->
        assertEquals(0.92f, get().dictionarySettings.cardBackgroundOpacity, "0.92 out of the box")
        assertOpacityReads(0.92f)
    }

    @Test
    fun `dragging the opacity slider left of its end lowers the stored opacity`() = dictionarySettingsTab { get ->
    ...
    @Test
    fun `the opacity slider reaches both ends of its range`() = dictionarySettingsTab { get ->
    ...
    @Test
    fun `the opacity slider is monotonic across its track`() = dictionarySettingsTab { get ->
     ...
     */
    @Test
    fun `the opacity slider leaves the transition duration alone`() = dictionarySettingsTab { get ->
    ...
    @Test
    fun `the fade in switch turns off and back on`() = dictionarySettingsTab { get ->
    ...
    @Test
    fun `the fade out switch turns off and back on`() = dictionarySettingsTab { get ->
    ...
    @Test
    fun `the duration readout starts on the stored value`() = dictionarySettingsTab { get ->
        assertEquals(500f, get().dictionarySettings.transitionDuration, "500ms out of the box")
        assertDurationReads(500f)
    }

    @Test
    fun `the duration slider reaches both ends of its range`() = dictionarySettingsTab { get ->
    ...
    @Test
    fun `dragging the duration slider right of its start raises the stored duration`() = dictionarySettingsTab { get ->
    ...
    @Test
    fun `the duration slider leaves the card opacity alone`() = dictionarySettingsTab { get ->
