...
class DictionarySettingsTabReferenceTest {
    ...
    @Test
    fun `the show switch turns the reference off and back on`() = dictionarySettingsTab { get ->
    ...
    fun `the reference colour field stores the confirmed hex`() {
        dictionarySettingsTab(initial = dictionarySettings { copy(referenceColor = "#334455") }) { get ->
    ...
    @Test
    fun `the reference shadow button stores the flag and reveals the shadow row`() = dictionarySettingsTab { get ->
    ...
    fun `the reference shadow size stores a new percentage`() {
        dictionarySettingsTab(initial = dictionarySettings { copy(referenceShadow = true, referenceShadowSize = 133) }) { get ->
    ...
    fun `the reference font dropdown stores the picked family`() {
        ...
        dictionarySettingsTab(initial = dictionarySettings { copy(referenceFontType = SENTINEL_FONT) }) { get ->
    ...
    @Test
    fun `the reference font size stores a new value`() = dictionarySettingsTab { get ->
    ...
    @Test
    fun `a reference font size outside the range is not stored`() = dictionarySettingsTab { get ->
