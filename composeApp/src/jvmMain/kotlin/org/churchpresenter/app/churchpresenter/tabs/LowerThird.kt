...
fun LowerThirdTab(
    ...
    LaunchedEffect(jsonContent) {
        if (jsonContent.isNotBlank()) { delay(COMPOSITION_LOAD_SETTLE_MS); isCompositionLoading = false }
    }
                        ...
                        when {
                            ...
                            atemSlots.isNotEmpty() -> {
                                ...
                                OutlinedTextField(
                                    ...
                                    onValueChange = {
                                        it.toIntOrNull()?.let { v -> atemSlot = (v - 1).coerceAtLeast(0) }
                                    },
                                    singleLine = true,
                                        ...
                                        modifier = Modifier.size(14.dp).initialPassClickable {
                                            SwingUtilities.invokeLater {
                                                ...
                                                if (result == JOptionPane.YES_OPTION) {
                                                    file.delete()
                                                    if (selectedFile?.absolutePath == file.absolutePath) selectedFile =
                                                        null
                                                    refreshKey++
                                                }
                                            }
                ...
                Text(
                    ...
                    style =
                        MaterialTheme.typography
                            .bodyMedium
                            .copy(fontWeight = if (selectedFile != null) FontWeight.Medium else FontWeight.Normal),
                ...
                if (atemConfigured && atemEverConnected) {
                    ...
                    if (appSettings.atemSettings.quickUpload) {
                        ...
                        val quickClipVariant =
                            if (jsonContent.isNotBlank()) atemVariant(isClip = true, useDetectedFps = false) else null
                        val quickClipCapacity = appSettings.atemSettings.detectedClipMaxFrames.getOrNull(clipSlot)
                        val quickClipTooLong =
                            quickClipVariant != null
                                && quickClipCapacity != null
                        ...
                        Tooltip(quickClipLabel) {
                            FilledIconButton(
                                onClick = { quickClipVariant?.let { startAtemUpload(
                                    it,
                                    clipSlot,
                                    closeDialogOnSuccess = false
                                ) } },
                        ...
                        Tooltip(if (atemReachable) stringResource(
                            Res.string.atem_send_to_atem
