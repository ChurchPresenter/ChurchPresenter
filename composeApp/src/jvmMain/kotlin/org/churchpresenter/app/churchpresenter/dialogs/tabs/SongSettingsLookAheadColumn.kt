...
internal fun LookAheadColumn(
        ...
        SingleChoiceSegmentedButtonRow(modifier = Modifier.fillMaxWidth().height(28.dp)) {
            laFsModes.forEachIndexed { index, (mode, label) ->
                SegmentedButton(
                    ...
                    onClick = {
                        onSettingsChange { s ->
                            s.copy(songSettings = s.songSettings.copy(lookAheadLanguageDisplay = mode))
                        }
                    },
    ...
    SettingRow(stringResource(Res.string.horizontal_alignment), width = 200.dp) {
        HorizontalAlignmentButtons(
            ...
            onAlignmentChange = { storedValue ->
                onSettingsChange { s ->
                    s.copy(songSettings = s.songSettings.copy(lookAheadHorizontalAlignment = storedValue))
                }
            },
    ...
    SettingRow(stringResource(Res.string.font_size)) {
        Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(4.dp)) {
            NumberSettingsTextField(
                ...
                onValueChange = {
                    onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lookAheadFontSize = it)) }
                },
                range = 8..150
            )
            TooltipArea(
                ...
                LabeledCheckbox(
                    ...
                    onCheckedChange = {
                        onSettingsChange { s ->
                            s.copy(songSettings = s.songSettings.copy(lookAheadFontSizeAutoFit = it))
                        }
                    },
    ...
    Row(horizontalArrangement = Arrangement.spacedBy(8.dp), verticalAlignment = Alignment.CenterVertically) {
        ColorPickerField(
            ...
            onColorChange = { color ->
                onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lookAheadColor = color)) }
            }
        )
        TextStyleButtons(
            ...
            onItalicChange = {
                onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lookAheadItalic = it)) }
            },
            onUnderlineChange = {
                onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lookAheadUnderline = it)) }
            },
            onShadowChange = {
                onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lookAheadShadow = it)) }
            }
        )
    }
    AnimatedVisibility(visible = settings.songSettings.lookAheadShadow) {
        ShadowDetailRow(
            ...
            onColorChange = {
                onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lookAheadShadowColor = it)) }
            },
            onSizeChange = {
                onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lookAheadShadowSize = it)) }
            },
            onOpacityChange = {
                onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lookAheadShadowOpacity = it)) }
            }
        )
    }
    ...
    SettingRow(stringResource(Res.string.font_size)) {
        Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(4.dp)) {
            NumberSettingsTextField(
                ...
                onValueChange = {
                    onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lookAheadNextFontSize = it)) }
                },
                range = 8..150
            )
            TooltipArea(
                ...
                LabeledCheckbox(
                    ...
                    onCheckedChange = {
                        onSettingsChange { s ->
                            s.copy(songSettings = s.songSettings.copy(lookAheadNextFontSizeAutoFit = it))
                        }
                    },
                    controlModifier = Modifier.size(24.dp),
                    label = stringResource(Res.string.auto_fit),
    ...
    FontSettingsDropdown(
        ...
        fonts = availableFonts,
        onValueChange = {
            onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lookAheadNextFontType = it)) }
        }
    )
    Spacer(Modifier.height(4.dp))
    Row(horizontalArrangement = Arrangement.spacedBy(8.dp), verticalAlignment = Alignment.CenterVertically) {
        ColorPickerField(
            ...
            onColorChange = { color ->
                onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lookAheadNextColor = color)) }
            }
        )
        TextStyleButtons(
            ...
            onBoldChange = {
                onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lookAheadNextBold = it)) }
            },
            onItalicChange = {
                onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lookAheadNextItalic = it)) }
            },
            onUnderlineChange = {
                onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lookAheadNextUnderline = it)) }
            },
            onShadowChange = {
                onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lookAheadNextShadow = it)) }
            }
        )
    }
    AnimatedVisibility(visible = settings.songSettings.lookAheadNextShadow) {
        ShadowDetailRow(
            ...
            onColorChange = {
                onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lookAheadNextShadowColor = it)) }
            },
            onSizeChange = {
                onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lookAheadNextShadowSize = it)) }
            },
            onOpacityChange = {
                onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lookAheadNextShadowOpacity = it)) }
            }
        )
    }
        ...
        SingleChoiceSegmentedButtonRow(modifier = Modifier.fillMaxWidth().height(28.dp)) {
            laLtModes.forEachIndexed { index, (mode, label) ->
                SegmentedButton(
                    ...
                    onClick = {
                        onSettingsChange { s ->
                            s.copy(songSettings = s.songSettings.copy(lowerThirdLookAheadLanguageDisplay = mode))
                        }
                    },
    ...
    SettingRow(stringResource(Res.string.horizontal_alignment), width = 200.dp) {
        HorizontalAlignmentButtons(
            ...
            onAlignmentChange = { storedValue ->
                onSettingsChange { s ->
                    s.copy(songSettings = s.songSettings.copy(lowerThirdLookAheadHorizontalAlignment = storedValue))
                }
            },
            leftValue = Constants.LEFT,
            centerValue = Constants.CENTER,
    ...
    SettingRow(stringResource(Res.string.font_size)) {
        Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(4.dp)) {
            NumberSettingsTextField(
                ...
                onValueChange = {
                    onSettingsChange { s ->
                        s.copy(songSettings = s.songSettings.copy(lowerThirdLookAheadFontSize = it))
                    }
                },
                range = 8..150
            )
            TooltipArea(
                ...
                LabeledCheckbox(
                    ...
                    onCheckedChange = {
                        onSettingsChange { s ->
                            s.copy(songSettings = s.songSettings.copy(lowerThirdLookAheadFontSizeAutoFit = it))
                        }
                    },
                    controlModifier = Modifier.size(24.dp),
                    label = stringResource(Res.string.auto_fit),
    ...
    FontSettingsDropdown(
        ...
        fonts = availableFonts,
        onValueChange = {
            onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lowerThirdLookAheadFontType = it)) }
        }
    )
    Spacer(Modifier.height(4.dp))
    Row(horizontalArrangement = Arrangement.spacedBy(8.dp), verticalAlignment = Alignment.CenterVertically) {
        ColorPickerField(
            ...
            onColorChange = { color ->
                onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lowerThirdLookAheadColor = color)) }
            }
        )
        TextStyleButtons(
            ...
            onBoldChange = {
                onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lowerThirdLookAheadBold = it)) }
            },
            onItalicChange = {
                onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lowerThirdLookAheadItalic = it)) }
            },
            onUnderlineChange = {
                onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lowerThirdLookAheadUnderline = it)) }
            },
            onShadowChange = {
                onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lowerThirdLookAheadShadow = it)) }
            }
        )
    }
    AnimatedVisibility(visible = settings.songSettings.lowerThirdLookAheadShadow) {
        ShadowDetailRow(
            ...
            onColorChange = {
                onSettingsChange { s ->
                    s.copy(songSettings = s.songSettings.copy(lowerThirdLookAheadShadowColor = it))
                }
            },
            onSizeChange = {
                onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lowerThirdLookAheadShadowSize = it)) }
            },
            onOpacityChange = {
                onSettingsChange { s ->
                    s.copy(songSettings = s.songSettings.copy(lowerThirdLookAheadShadowOpacity = it))
                }
            }
        )
    }
    ...
    SettingRow(stringResource(Res.string.font_size)) {
        Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(4.dp)) {
            NumberSettingsTextField(
                ...
                onValueChange = {
                    onSettingsChange { s ->
                        s.copy(songSettings = s.songSettings.copy(lowerThirdLookAheadNextFontSize = it))
                    }
                },
                range = 8..150
            )
            TooltipArea(
                ...
                LabeledCheckbox(
                    ...
                    onCheckedChange = {
                        onSettingsChange { s ->
                            s.copy(songSettings = s.songSettings.copy(lowerThirdLookAheadNextFontSizeAutoFit = it))
                        }
                    },
                    controlModifier = Modifier.size(24.dp),
                    label = stringResource(Res.string.auto_fit),
    ...
    FontSettingsDropdown(
        ...
        fonts = availableFonts,
        onValueChange = {
            onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lowerThirdLookAheadNextFontType = it)) }
        }
    )
    Spacer(Modifier.height(4.dp))
    Row(horizontalArrangement = Arrangement.spacedBy(8.dp), verticalAlignment = Alignment.CenterVertically) {
        ColorPickerField(
            ...
            onColorChange = { color ->
                onSettingsChange { s ->
                    s.copy(songSettings = s.songSettings.copy(lowerThirdLookAheadNextColor = color))
                }
            }
        )
        TextStyleButtons(
            ...
            onBoldChange = {
                onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lowerThirdLookAheadNextBold = it)) }
            },
            onItalicChange = {
                onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lowerThirdLookAheadNextItalic = it)) }
            },
            onUnderlineChange = {
                onSettingsChange { s ->
                    s.copy(songSettings = s.songSettings.copy(lowerThirdLookAheadNextUnderline = it))
                }
            },
            onShadowChange = {
                onSettingsChange { s -> s.copy(songSettings = s.songSettings.copy(lowerThirdLookAheadNextShadow = it)) }
            }
        )
    }
    AnimatedVisibility(visible = settings.songSettings.lowerThirdLookAheadNextShadow) {
        ShadowDetailRow(
            ...
            onColorChange = {
                onSettingsChange { s ->
                    s.copy(songSettings = s.songSettings.copy(lowerThirdLookAheadNextShadowColor = it))
                }
            },
            onSizeChange = {
                onSettingsChange { s ->
                    s.copy(songSettings = s.songSettings.copy(lowerThirdLookAheadNextShadowSize = it))
                }
            },
            onOpacityChange = {
                onSettingsChange { s ->
                    s.copy(songSettings = s.songSettings.copy(lowerThirdLookAheadNextShadowOpacity = it))
                }
            }
        )
    }
