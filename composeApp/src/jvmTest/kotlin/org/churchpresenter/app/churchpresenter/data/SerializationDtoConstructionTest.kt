...
class SerializationDtoConstructionTest {
    ...
    fun `pexels dtos round-trip their fields`() {
        ...
        val file = StockMediaClient.PexelsVideoFile(link = "https://v/hd.mp4",
            quality = "hd",
            file_type = "video/mp4",
            width = 1920)
    ...
    fun `pixabay dtos round-trip their fields`() {
        val photo = StockMediaClient.PixabayPhoto(id = 999L,
    ...
    fun `dtos with an omitted field fall back to their default`() {
        ...
        assertEquals(null,
            StockMediaClient.PixabayVideo(id = 1L, videos = StockMediaClient.PixabayVideoFiles()).picture_id)
