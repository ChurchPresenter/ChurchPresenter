 ...
 * the client actually builds one by hand today, but the plain constructor and its accessors are
 * still real, callable code, so it is worth pinning that a value put in comes back out unchanged.
 *
 * Planning Center's own DTOs are the same shape and are pinned the same way, in
 * `:planning-center`'s PlanningCenterDtoConstructionTest — they are `internal` to that module now,
