...
class WebTabTest {
    ...
    fun `clicking the bookmark star records the normalised URL as a new bookmark`() = webTab { _, reports ->
        ...
        assertEquals(listOf(WebBookmark(url = "https://example.com", title = "https://example.com")),
    ...
    @Test
    fun `clicking Add to Schedule reports the normalised URL and a title falling back to the URL`() = webTab { _,
        reports ->
        onNodeWithText(WebLabel.URL_PLACEHOLDER_DEFAULT).performTextReplacement("example.com")
    ...
    @Test
    fun `Add to Schedule is absent when the tab is given no callback for it`() = webTab(includeAddToSchedule = false) { _,
        _ ->
    ...
    @Test
    fun `Back Forward Refresh and Clear Cache are all present and clickable with no browser attached`() = webTab { _,
        _ ->
