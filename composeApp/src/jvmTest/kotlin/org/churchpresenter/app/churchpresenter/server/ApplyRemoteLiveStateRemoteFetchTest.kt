...
class ApplyRemoteLiveStateRemoteFetchTest {
    ...
    fun `a live picture is fetched, cached locally, and displayed by its cache path`() {
        ...
        val presenter =
            apply(LiveStateDto(contentType = "PICTURES", pictureFolderId = "apply-remote-fetch-1", pictureIndex = 0))
    ...
    fun `a second apply of the same picture reuses the cache instead of re-fetching`() {
        ...
        val first =
            apply(LiveStateDto(contentType = "PICTURES", pictureFolderId = "apply-remote-fetch-2", pictureIndex = 0))
