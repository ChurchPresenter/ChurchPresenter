...
class PresentationPresenterDrawTest {
    ...
    fun `a single full-frame layer fills the whole canvas when aspect ratios match`() {
        val frame = presentationFrame(listOf(placedLayer(Color.Red, width = 100, height = 100)),
            frameWidthPx = 100,
            frameHeightPx = 100)
    ...
    fun `a mismatched aspect ratio letterboxes with the black background`() {
        val frame = presentationFrame(listOf(placedLayer(Color.Red, width = 100, height = 200)),
            frameWidthPx = 100,
            frameHeightPx = 200)
