...
class CompanionServerAtemUploadTest {
    ...
    companion object {
         ...
         */
        /**
         * The frames the cache is filled with. The routes under test upload what the cache holds;
         * what drew it is not their business, and a fake keeps skia off this module's classpath.
         */
        private val RENDERER = FakeLottieFrameRenderer()
        ...
        @BeforeClass
        fun startServer() {
            // The one JVM-wide user.home consumer this suite has left: the Instance Link log path.
            // In :composeApp it also had to pin skiko's native-library unpack directory, because the
            // real renderer draws with skia. Here the frames come from FakeLottieFrameRenderer and
            // skia is not on the classpath at all.
            LogHomeLatch.latch()
            ...
            val cached = runBlocking { LottieRenderCache.prepare(LOTTIE, variant, RENDERER).await() }
            ...
            val cachedClip = runBlocking { LottieRenderCache.prepare(LOTTIE, clipVariant, RENDERER).await() }
            ...
            server = CompanionServer(CompanionHost(lottieRenderer = RENDERER))
