...
class CompanionServerPreloadTest {
    ...
    companion object {
        ...
        fun startServer() {
            ...
            server = CompanionServer(CompanionHost(loadSongs = ::readSpsLibrary))
        ...
        /**
         * Reads the `.sps` line format [songFolder] writes, and throws on anything else.
         *
         * `preloadData` does not parse songs itself — it walks the folder and hands each file to
         * `CompanionHost.loadSongs`, which in the app is `Songs.loadFromSpsAppend` (that reader also
         * handles the SQLite flavour of `.sps`, and holds Compose snapshot state, so it stays in
         * `:composeApp` and is covered by its own suite). What is under test here is the walking:
         * which files are offered, what happens when one of them throws, and that the catalog is
         * added to rather than replaced. So this reads back exactly what the fixture wrote, and
         * refusing a file that is not in that shape is the point — two tests depend on it throwing.
         */
        @JvmStatic
        private fun readSpsLibrary(file: File): List<SongItem> {
            val lines = file.readLines()
            require(lines.firstOrNull() == "##SongPresenter") { "not an .sps library: ${file.name}" }
            val songbook = lines.getOrNull(1)?.removePrefix("##").orEmpty()
            return lines.drop(2).filter { it.isNotBlank() }.map { line ->
                val f = line.split("#\$#")
                require(f.size >= SPS_FIELDS) { "malformed .sps row in ${file.name}" }
                SongItem(
                    number = f[0], title = f[1], songbook = songbook, tune = f[3],
                    author = f[4], composer = f[5], lyrics = f[6].split("\n"),
                )
            }
        }

        /** number, title, category, key, author, composer, lyrics — see `row()` below. */
        private const val SPS_FIELDS = 7
