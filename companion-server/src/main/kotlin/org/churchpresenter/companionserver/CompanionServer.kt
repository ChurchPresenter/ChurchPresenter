...
class CompanionServer(
    /**
     * What this server may ask of the app around it. Defaulted so a test starts a real server with
     * no app at all; `main.kt` builds the real one. See [CompanionHost].
     */
    val host: CompanionHost = CompanionHost(),
) {
    ...
    /**
     * Whether a presentation is the live content, as every connected client is told it is.
     *
     * A read-only view of the flag [updatePresentationLiveStatus] sets. The backing field is
     * module-internal because the routes write it per request; this is how the app — and the wiring
     * test that drives `LiveStatusWiring` — reads it back without reaching into the module.
     */
