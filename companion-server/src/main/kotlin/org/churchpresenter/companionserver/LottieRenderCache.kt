...
object LottieRenderCache {
     ...
     */
    fun prepare(lottieJson: String, variant: Variant, renderer: LottieFrameRenderer): Deferred<File> {
        val key = keyFor(lottieJson, variant)
                ...
                scope.async {
                    ...
                    if (!dest.exists()) {
                        renderMutex.withLock {
                            if (!dest.exists()) renderToFile(lottieJson, variant, dest, key, renderer)
                        }
                    }
     ...
     */
    fun ensureForFolder(folderPath: String, atem: AtemSettings?, renderer: LottieFrameRenderer) {
        if (folderPath.isEmpty()) return
        scope.launch(Dispatchers.IO) {
            File(folderPath).takeIf { it.isDirectory }
                ?.listFiles { f -> f.extension.lowercase() == "json" && isLottieFile(f) }
                ?.forEach { ensureForFile(it, atem, renderer) }
        }
    }
     ...
     */
    fun ensureForFile(file: File, atem: AtemSettings?, renderer: LottieFrameRenderer) {
        scope.launch(Dispatchers.IO) {
            try {
                val json = file.readText()
                desktopVariant(json, atem)?.let { prepare(json, it, renderer) }
                if (atem != null && atem.host.isNotBlank()) {
                    prepare(json, atemVariant(json, atem, clip = false), renderer)
                    if (clipFrameCount(json, atem.clipFps) != null) {
                        prepare(json, atemVariant(json, atem, clip = true), renderer)
                    }
                }
            } catch (e: Exception) {
    ...
    private suspend fun renderToFile(
        ...
        try {
            DataOutputStream(BufferedOutputStream(FileOutputStream(tmp))).use { out ->
                ...
                renderer.withSession(
                    v.width, v.height, lottieJson, initialProgress = if (v.clip) 0f else 0.5f,
                ) { renderFrame ->
