...
class DetectionEngineRefEventTest {
    ...
    fun `an explicit reference is trusted outright`() {
        val event: engine.engine.ScriptureEvent = assertNotNull(engine().processTranscription("live", "turn to John chapter 3 verse 16").firstOrNull())
    ...
    fun `a sticky continuation is scored by agreement rather than trusted`() {
        ...
        val event: engine.engine.ScriptureEvent = assertNotNull(e.processTranscription("live", "verse 17").firstOrNull())
