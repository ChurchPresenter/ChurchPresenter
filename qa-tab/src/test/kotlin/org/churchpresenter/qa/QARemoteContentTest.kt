 ...
 *
 * Left uncovered: the `DialogWindow` call and the grow-to-fit effect, neither of which runs headless.
 *
 * The two opacity `SlimSlider`s *are* covered, by coordinate. `SlimSlider` publishes no semantics of
 * its own — it is a `Canvas` behind a tap detector — so each is found through its own readout, which
 * `SlimSlider` places exactly 10.dp off the slider's right edge. The two readouts are told apart by
 * giving them different percentages in the fixture.
 */
class QARemoteContentTest {
    ...
    private companion object {
        ...
        /** `SlimSlider`'s own `Row(spacedBy(10.dp))` between the track and its readout. */
        const val SLIDER_READOUT_GAP_PX = 10f
    }
    ...
    /**
     * Clicks a point on the track of the slider whose readout says [readout], [fromRightEdge] pixels
     * in from its right end.
     *
     * The readout is a plain `Text` 10.dp past the track, so its left edge less that gap is the
     * track's right edge. Clicking near it lands high in the range; clicking further left lands
     * lower, which is what a test asserts on rather than an exact percentage.
     */
    private fun ComposeUiTest.clickSliderTrack(readout: String, fromRightEdge: Float) {
        val node = onAllNodes(hasText(readout))
            .fetchSemanticsNodes(atLeastOneRootRequired = false)
            .firstOrNull { it.boundsInRoot.width > 0f }
            ?: error("no slider readout says \"$readout\"")
        val trackEnd = node.boundsInRoot.left - SLIDER_READOUT_GAP_PX
        onRoot().performMouseInput { click(Offset(trackEnd - fromRightEdge, node.boundsInRoot.center.y)) }
        waitForIdle()
    }

    @Test
    fun `dragging the QR background opacity slider changes only that setting`() =
        qaRemote(initialQaSettings = QASettings(qrBackgroundOpacity = 90, backgroundOpacity = 20)) { h ->
            clickSliderTrack("90%", fromRightEdge = 2f)

            // Not exactly 100: the click lands two pixels inside the track, and the slider maps
            // position to percentage, so the top of the range is "as opaque as it gets from here".
            assertTrue(
                h.settings.qrBackgroundOpacity >= 95,
                "the right-hand end is near-full opacity, got ${h.settings.qrBackgroundOpacity}",
            )
            assertEquals(20, h.settings.backgroundOpacity, "the other slider must not move with it")
        }

    @Test
    fun `clicking further left along the QR opacity slider sets a lower value`() =
        qaRemote(initialQaSettings = QASettings(qrBackgroundOpacity = 90, backgroundOpacity = 20)) { h ->
            clickSliderTrack("90%", fromRightEdge = 2f)
            val atTheRightEnd = h.settings.qrBackgroundOpacity

            clickSliderTrack("$atTheRightEnd%", fromRightEdge = 40f)

            assertTrue(
                h.settings.qrBackgroundOpacity < atTheRightEnd,
                "further left must be less opaque (${h.settings.qrBackgroundOpacity} vs $atTheRightEnd)",
            )
        }

    @Test
    fun `dragging the card background opacity slider changes only that setting`() =
        qaRemote(initialQaSettings = QASettings(qrBackgroundOpacity = 90, backgroundOpacity = 20)) { h ->
            clickSliderTrack("20%", fromRightEdge = 2f)

            assertTrue(
                h.settings.backgroundOpacity >= 95,
                "the right-hand end is near-full opacity, got ${h.settings.backgroundOpacity}",
            )
            assertEquals(90, h.settings.qrBackgroundOpacity, "the QR slider must not move with it")
        }
}
