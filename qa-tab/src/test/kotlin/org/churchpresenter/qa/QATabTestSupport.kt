...
internal fun qaTab(
    ...
    /** Where the export dialog would put the file; `null` stands for the operator cancelling. */
    exportTo: Path? = null,
    /** The file the import dialog would return; `null` stands for the operator cancelling. */
    importFrom: Path? = null,
    block: ComposeUiTest.(qa: QAManager, output: FakeQaOutput, reports: QAReports) -> Unit,
) {
    val realHome = System.getProperty("user.home")
    val tempHome: File = Files.createTempDirectory("cp-qa-tab").toFile()
    System.setProperty("user.home", tempHome.absolutePath)
    val qa = QAManager()
    val output = FakeQaOutput()
            ...
            setContent {
                ThemedForTest(themeMode) {
                    ...
                    QATab(
                        ...
                        chooseExportFile = { _, _ -> exportTo },
                        chooseImportFile = { importFrom },
                    )
                    }
                    }
                }
            }
            block(qa, output, reports)
        }
    } finally {
...
internal object QALabel {
    ...
    const val NO_HISTORY = "No history from previous sessions"
