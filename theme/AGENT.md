...
| `AppShape.kt` | `AppShape(...)` — the app's corner, drawn as a `SquircleShape` (a corner that eases into its edges) at `CORNER_SCALE` of the radius written — and `SquircleShape` itself. Percent corners are not scaled, so a pill stays a pill |
